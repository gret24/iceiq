#!/usr/bin/env python3
"""
IceIQ Fast Pipeline (EasyOCR + 3가지 최적화)
변경: allowlist 추가, confirmed_tracks 캐시, bbox 높이<50px 스킵
...
def step_ocr(all_tracks: dict | None = None, roster_mgr=None):
    ...
    # [캐시] confirmed_tracks: {track_id: jersey_number}
    confirmed_tracks: dict[int, str] = {}
    ocr_votes: dict[int, dict] = {}
    ocr_call_count = 0
    ocr_skip_cache = 0
    ocr_skip_bbox  = 0
    ...
    for i, (tid, frame_list) in enumerate(sorted(track_frames.items()), 1):
        ...
        if tid in confirmed_tracks:
            jersey_map[str(tid)] = {"jersey": confirmed_tracks[tid], "team": "UNKNOWN"}
            ocr_skip_cache += 1
            continue
        ...
        for filename, track in frame_list[:MAX_VOTE_FRAMES * 3]:
            ...
            # [개선 1] EasyOCR → 다수결 투표
            ocr_call_count += 1; result = _easyocr_jersey(reader, crop_arr)
            if result:
                num, conf = result
    ...
    total_skip = ocr_skip_cache + ocr_skip_bbox
    total_att = ocr_call_count + total_skip
    reduction = total_skip / total_att * 100 if total_att > 0 else 0
    print(f"\n  [Fast 통계] OCR호출:{ocr_call_count}회 | 캐시히트:{ocr_skip_cache}회 | bbox스킵:{ocr_skip_bbox}회 | 감소율:{reduction:.1f}%")
    return jersey_map
